...
namespace FrostySdk.Managers
{
    public partial class AssetManager
    {
        ...
        {
            private void LoadSuperBundle(AssetManager parent, BinarySbDataHelper helper, string superBundleName)
            {
                ...
                DbObject baseToc = parent.ProcessTocChunks(string.Format("native_data/{0}.toc", superBundleName),
                    helper, true);
                if (baseToc == null)
                    return;
                bool isBinarySuperBundle = baseToc.GetValue<bool>("alwaysEmitSuperBundle") || (ProfilesLibrary.DataVersion == (int)ProfileVersion.PlantsVsZombiesGardenWarfare ||
                    ProfilesLibrary.DataVersion == (int)ProfileVersion.PlantsVsZombiesGardenWarfare2);

                parent.WriteToLog("Loading data ({0})", superBundleName);

                // process patch toc
                DbObject patchToc =
                    parent.ProcessTocChunks(string.Format("native_patch/{0}.toc", superBundleName), helper);
                ...
                NativeReader baseMf = new NativeReader(new FileStream(
                    parent.m_fileSystem.ResolvePath(string.Format("native_data/{0}.sb", superBundleName)),
                    FileMode.Open, FileAccess.Read));
                NativeReader patchMf = baseMf;

                if (patchFileExists)
                    patchMf = new NativeReader(new FileStream(
                        parent.m_fileSystem.ResolvePath(string.Format("native_patch/{0}.sb", superBundleName)),
                        FileMode.Open, FileAccess.Read));
                ...
                foreach (DbObject bundle in patchBundleList)
                {
                    ...
                    parent.AddBundle(new BundleEntry()
                    {
                        Name = bundleName, SuperBundleId = parent.m_superBundles.Count - 1
                    });
                    ...
                    if (isBinarySuperBundle)
                    {
                        ...
                        {
                            // get base stream
                            BaseBundleInfo bi = baseBundles.TryGetValue(bundleName, out var baseBundle) ? baseBundle : null;
                            ...
                            using (BinarySbReader reader = new LegacyBinarySbReader(baseStream, stream,
                                       parent.m_fileSystem.CreateDeobfuscator()))
                                sb = reader.ReadDbObject();

                            DbObject baseSb = null;
                            if (bi != null)
                            {
                                using (BinarySbReader reader = new LegacyBinarySbReader(
                                           baseMf.CreateViewStream(bi.Offset, bi.Size), bi.Offset,
                                           parent.m_fileSystem.CreateDeobfuscator()))
                        ...
                        {
                            // binary super bundle
                            using (BinarySbReader reader = new LegacyBinarySbReader(stream, offset,
                                       parent.m_fileSystem.CreateDeobfuscator()))
                                sb = reader.ReadDbObject();
                        }
                    }
            ...
            public void Load(AssetManager parent, BinarySbDataHelper helper)
            {
                ...
                var tasks = parent.m_fileSystem.SuperBundles.Select(x =>
                    Task.Run(
                        () => LoadSuperBundle(parent, helper, x)
                    ));
